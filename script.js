...
const loadFin=*=>{Object.entries(JSON.parse(localStorage.getItem("fin")||"{}")).forEach((\[k,v])=>id(k).value=v)};
const saveFin=*=>localStorage.setItem("fin",JSON.stringify(Object.fromEntries(keys.map(k=>\[k,id(k).value]))));
